...
REACTION_CLI_VERSION=$(create-reaction-app -v | grep Reaction);
if [[ "$REACTION_CLI_VERSION" =~ "Create-reaction-app CLI:" ]]; then
  echo "Success: Create-reaction-app CLI found";
else
  echo "Error: Create-reaction-app CLI not installed";
  exit 1;
fi